...
export const useEcommerce = () => {
    ...
    if (!context) {
        throw new Error(
            "useEcommerce must be used within an EcommerceProvider"
        );
    }
    return context;
};
...
export const EcommerceProvider = ({ children }) => {
    ...
    // Initialize cart and wishlist from user data
    useEffect(() => {
        if (user?.data) {
            setCartItems(user.data.cart || []);
            setWishlistItems(user.data.wishlist || []);
            setAddresses(user.data.addressInfo || []);
            // Payment methods would come from user data or separate API
            setPaymentMethods([]);
        }
    }, [user]);
